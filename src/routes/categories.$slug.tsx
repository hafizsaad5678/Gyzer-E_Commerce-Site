...
const opts = (slug: string) =>
  queryOptions({
    ...
    queryFn: async () => {
      const { data: cat, error: cErr } = await supabase.from("categories").select("*").eq("slug", slug).eq("is_active", true).maybeSingle();
      if (cErr) throw cErr;
      if (!cat) throw notFound();
      const { data: products } = await supabase
        .from("products")
        .select("id,slug,name,brand,price_pkr,discount_price_pkr,cover_image_url,capacity_liters,warranty_months,stock")
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="container-page py-32 text-center">
        ...
        <Link to="/categories" className="text-copper underline">All categories</Link>
      </div>
    </SiteLayout>
  ),
});
  ...
  return (
    <SiteLayout>
      <section className="container-page py-12 md:py-16">
        ...
        <div className="my-6">
          <div className="text-xs uppercase tracking-wider text-copper font-semibold mb-2">Category</div>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {products.map((p) => <ProductCard key={p.id} p={p as ProductCardData} fallbackImg={catImg[slug]} />)}
          </div>
        )}
      </section>
    </SiteLayout>
