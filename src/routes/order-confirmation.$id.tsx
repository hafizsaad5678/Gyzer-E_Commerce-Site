...
function OrderConfirmation() {
  ...
  if (loading) return <SiteLayout><div className="container-page py-20 text-muted-foreground">Loading…</div></SiteLayout>;
  if (!order) return <SiteLayout><div className="container-page py-32 text-center"><h1 className="text-display text-3xl mb-3">Order not found</h1><Link to="/" className="text-copper underline">Home</Link></div></SiteLayout>;
    ...
    <SiteLayout>
      <section className="container-page py-12 max-w-3xl">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground">Order <span className="font-mono text-foreground">{order.order_number}</span> · We've sent confirmation to {order.email}.</p>
        </div>
        ...
        <div className="surface-card p-6 mb-6">
            ...
            {items.map((i) => (
              <li key={i.id} className="flex justify-between py-3 text-sm">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{i.product_sku} · Qty {i.quantity}</div>
                </div>
          ...
          <dl className="border-t border-border mt-4 pt-4 space-y-2 text-sm">
            ...
            {Number(order.discount_pkr) > 0 && <Row k="Discount" v={`-${formatPKR(order.discount_pkr)}`} />}
            <Row k="Shipping" v={Number(order.shipping_pkr) === 0 ? "Free" : formatPKR(order.shipping_pkr)} />
        ...
        <div className="surface-card p-6 mb-8">
          ...
          <div className="text-sm">
            ...
            <div className="text-muted-foreground">
              {a?.line1}{a?.line2 ? `, ${a.line2}` : ""}, {a?.city}, {a?.province} {a?.postal_code ?? ""}, {a?.country}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          <Link to="/account/orders" className="rounded-md bg-primary px-5 py-3 text-sm font-medium text-primary-foreground">View my orders</Link>
