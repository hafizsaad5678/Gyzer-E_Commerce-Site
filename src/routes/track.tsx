...
function TrackOrder() {
  ...
  async function handleSearch(e: React.FormEvent) {
    e.preventDefault();
    if (!orderNum.trim() || !email.trim()) return toast.error("Please enter both Order Number and Email");
    ...
    <SiteLayout>
      <section className="container-page py-16 md:py-24 max-w-4xl mx-auto">
        <div className="text-center mb-10">
          <div className="text-xs uppercase tracking-wider text-copper font-semibold mb-2">Order Support</div>
          <h1 className="text-display text-4xl md:text-5xl mb-4">Track your order</h1>
          <p className="text-muted-foreground">Enter your order number and email address to check the current status.</p>
        </div>
        ...
        <div className="surface-card p-6 md:p-8 max-w-xl mx-auto mb-12">
          <form onSubmit={handleSearch} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full inline-flex items-center justify-center gap-2 rounded-md bg-copper px-5 py-3 text-sm font-semibold text-copper-foreground hover:opacity-90 disabled:opacity-50 mt-2"
            >
        ...
        {order && (
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="surface-card p-6 md:p-8 space-y-8">
              <h2 className="text-display text-2xl mb-6">Status: <span className="capitalize text-copper">{order.status}</span></h2>
              ...
              {order.tracking_number && order.status !== "pending" && order.status !== "processing" && (
                <div className="mt-8 p-4 rounded-md bg-secondary/50 border border-border">
                  <div className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-1">Tracking Information</div>
                  <div className="text-lg font-medium">{order.tracking_number}</div>
                  {order.courier_name && <div className="text-sm text-muted-foreground mt-1">Courier: {order.courier_name}</div>}
                </div>
              )}
            </div>
            ...
            <div className="surface-card p-6 md:p-8 h-fit space-y-6">
              ...
              <dl className="space-y-2 text-sm pt-4 border-t border-border">
                <div className="flex justify-between"><dt className="text-muted-foreground">Subtotal</dt><dd>{formatPKR(order.subtotal_pkr)}</dd></div>
                {order.discount_pkr > 0 && <div className="flex justify-between text-copper font-medium"><dt>Discount</dt><dd>-{formatPKR(order.discount_pkr)}</dd></div>}
                <div className="flex justify-between"><dt className="text-muted-foreground">Shipping</dt><dd>{order.shipping_pkr === 0 ? "Free" : formatPKR(order.shipping_pkr)}</dd></div>
                <div className="flex justify-between pt-3 border-t border-border text-base font-semibold">
                  <dt>Total</dt><dd>{formatPKR(order.total_pkr)}</dd>
                </div>
              </dl>
            </div>
          </div>
...
function TimelineStep({ active, icon, title, desc, date }: { active: boolean; icon: React.ReactNode; title: string; desc: string; date?: string }) {
  return (
    <div className={`relative flex items-start justify-between gap-4 ${active ? "opacity-100" : "opacity-40"}`}>
      <div className="absolute left-0 -ml-[21px] flex h-6 w-6 items-center justify-center rounded-full border-2 bg-background border-border">
        {active ? <div className="text-copper">{icon}</div> : <div className="h-1.5 w-1.5 rounded-full bg-muted-foreground" />}
      </div>
      <div>
        <h4 className={`text-sm font-semibold ${active ? "text-foreground" : "text-muted-foreground"}`}>{title}</h4>
