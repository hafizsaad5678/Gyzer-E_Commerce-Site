...
function Categories() {
 ...
 Each category is engineered for a different home setup. Not sure?{" "}
 <Link to="/contact" className="text-copper underline">
 Ask us.
 </Link>
 </p>
 ...
 to="/categories/$slug"
 params={{ slug: c.slug }}
 className="surface-card overflow-hidden grid md:grid-cols-[40%_1fr] group hover:shadow-[var(--shadow-elevated)] transition"
 >
 ...
 <h2 className="text-display text-2xl group-hover:text-copper transition-colors">
 {c.name}
 </h2>
 <div>
 <span className="inline-flex items-center rounded-full bg-copper/10 px-2.5 py-0.5 text-[11px] font-semibold tracking-wider text-copper uppercase group-hover:bg-copper group-hover:text-white transition-colors">
 {c.products?.[0]?.count ?? 0} Products
 </span>
 </div>
 <p className="text-sm text-muted-foreground">{c.description}</p>
 <div className="text-sm text-copper font-medium mt-auto pt-2">
 Browse {c.name} →
 </div>
 </div>
 </Link>
