...
export const Route = createFileRoute("/reset-password")({
  head: () => ({ meta: [{ title: "Reset password — Asif Brothers" }, { name: "robots", content: "noindex" }] }),
  ...
  return (
    <SiteLayout>
      <section className="container-page py-20 max-w-md">
        ...
        <p className="text-muted-foreground text-sm mb-8">{ready ? "Choose a new password for your account." : "Verifying reset link..."}</p>
        {ready && (
          <form onSubmit={submit} className="surface-card p-6 space-y-4">
            ...
            <button disabled={loading} className="w-full rounded-md bg-primary px-5 py-3 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-60">{loading ? "..." : "Update password"}</button>
          </form>
        )}
      </section>
    </SiteLayout>
