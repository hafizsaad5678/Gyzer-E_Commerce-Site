...
import { Package, ShoppingCart, TrendingUp, Users, BarChart2, FileDown } from "lucide-react";
import {
  AreaChart,
  Area,
  BarChart,
...
function filterOrdersByRange(orders: any[], key: RangeKey) {
...
function buildRevenueChart(orders: any[], range: RangeKey) {
  const days = range === "7d" ? 7 : range === "month" ? new Date().getDate() : 30;
...
function computeSales(orders: any[], products: any[] = []): SalesData {
  const grossSales = orders.reduce((s, o) => s + Number(o.subtotal_pkr ?? o.total_pkr ?? 0), 0);
  ...
  // cost_price_pkr not yet in DB — profit = revenue until column is added
  const estimatedCost = 0;
  return { grossSales, shippingCollected, discountsGiven, netSales, totalRevenue, estimatedCost, profit: totalRevenue };
}

const STATUS_COLORS: Record<string, string> = {
  pending: "#94a3b8", paid: "#22c55e", processing: "#f59e0b",
  shipped: "#3b82f6", delivered: "#10b981", cancelled: "#ef4444", refunded: "#f43f5e",
};
...
function Dashboard() {
  ...
  const [topProductsAll, setTopProductsAll] = useState<any[]>([]);
  const [stats, setStats] = useState({ products: 0, customers: 0, lowStock: [] as any[] });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const top = Object.values(productMap).sort((a, b) => b.revenue - a.revenue).slice(0, 6);

        setAllOrders(orders ?? []);
        setAllProducts(products ?? []);
        setRecentOrders(recent ?? []);
        setTopProductsAll(top);
        setStats({
  ...
  // ── Derived data (recalculated when range changes) ─────────────────────────
  const rangedOrders = useMemo(() => filterOrdersByRange(allOrders, range), [allOrders, range]);
  const salesData = useMemo(() => computeSales(rangedOrders, allProducts), [rangedOrders, allProducts]);
  const revenueData = useMemo(() => buildRevenueChart(rangedOrders, range), [rangedOrders, range]);
  const statusData = useMemo(() => buildStatusChart(rangedOrders), [rangedOrders]);
  const totalRevenue = useMemo(() => rangedOrders.reduce((s, o) => s + Number(o.total_pkr), 0), [rangedOrders]);
  ...
  function handleDownloadReport() {
    ...
    const reportData: ReportData = {
      period: {
        from: getRangeStart(range)?.toLocaleDateString("en-PK", { day: "2-digit", month: "short", year: "numeric" }) ?? "All time",
        to: now.toLocaleDateString("en-PK", { day: "2-digit", month: "short", year: "numeric" }),
      },
      sales: salesData,
      orderCounts: Object.fromEntries(
        Object.entries(computeStatusCounts(rangedOrders)).filter(([k]) => k !== "total"),
      ),
      topProducts: topProductsAll,
    ...
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => <div key={i} className="surface-card h-24" />)}
        </div>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className="surface-card p-12 text-center space-y-3">
        ...
        <button onClick={() => window.location.reload()} className="rounded-md bg-secondary hover:bg-copper text-foreground hover:text-copper-foreground transition-colors px-5 py-2 text-sm font-medium">
          Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {(Object.keys(RANGE_LABELS) as RangeKey[]).map((key) => (
              <button
                ...
                className={[
                  ...
                  range === key ? "bg-copper text-white" : "hover:bg-secondary",
                ].join(" ")}
              >
                {RANGE_LABELS[key]}
              </button>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md border border-input px-3 py-1.5 text-sm hover:bg-secondary transition-colors"
          >
      ...
      {/* KPI cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat icon={TrendingUp} label="Revenue" value={formatPKR(totalRevenue)} />
        <Stat icon={ShoppingCart} label="Orders" value={rangedOrders.length.toLocaleString()} />
      ...
      {/* Sales metrics */}
      <div>
        <h2 className="text-display text-xl mb-4">Sales Breakdown <span className="text-sm font-sans text-muted-foreground">— {RANGE_LABELS[range]}</span></h2>
      ...
      <div className="surface-card p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-display text-xl">Revenue — {RANGE_LABELS[range]}</h2>
          <span className="text-xs text-muted-foreground">{formatPKR(totalRevenue)} total</span>
        </div>
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={revenueData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
            ...
            <XAxis dataKey="date" tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} interval={Math.floor(revenueData.length / 6)} />
            <YAxis tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} tickFormatter={(v) => `Rs ${(v / 1000).toFixed(0)}k`} />
            <Tooltip
              contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }}
              formatter={(v: any) => [formatPKR(v), "Revenue"]}
            />
            <Area type="monotone" dataKey="revenue" stroke="#b87333" strokeWidth={2} fill="url(#revGrad)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="surface-card p-6">
          ...
          {topProductsAll.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={topProductsAll} layout="vertical" margin={{ left: 8, right: 16, top: 0, bottom: 0 }}>
                ...
                <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} width={100} />
                <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} formatter={(v: any) => [formatPKR(v), "Revenue"]} />
        ...
        {/* Order status pie */}
        <div className="surface-card p-6">
          <h2 className="text-display text-xl mb-5">Orders by status — {RANGE_LABELS[range]}</h2>
          {statusData.length === 0 ? (
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="surface-card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/orders" className="text-xs text-copper hover:underline">View all</Link>
          </div>
